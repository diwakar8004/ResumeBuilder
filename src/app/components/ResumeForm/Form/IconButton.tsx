...
export const MoveIconButton = ({
  ...
  return (
    <IconButton
      ...
      size={size}
    >
      <Icon className={`${sizeClassName} text-gray-400 dark:text-gray-500`} aria-hidden="true" />
      <span className="sr-only">{tooltipText}</span>
    </IconButton>
  );
};
...
export const DeleteIconButton = ({
  ...
  return (
    <IconButton onClick={onClick} tooltipText={tooltipText} size="small">
      <TrashIcon className="h-4 w-4 text-gray-400 dark:text-gray-500" aria-hidden="true" />
      <span className="sr-only">{tooltipText}</span>
    </IconButton>
  );
};
...
export const BulletListIconButton = ({
  ...
  return (
    <IconButton
      ...
      tooltipText={tooltipText}
      size="small"
      className={showBulletPoints ? "!bg-sky-100 dark:!bg-sky-900/30" : ""}
    >
      <ListBulletIcon
        className={`h-4 w-4 ${
          showBulletPoints ? "text-gray-700 dark:text-gray-200" : "text-gray-400 dark:text-gray-500"
        }`}
        aria-hidden="true"
      />
      <span className="sr-only">{tooltipText}</span>
