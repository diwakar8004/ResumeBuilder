...
describe("extract-profile tests - ", () => {
  ...
  it("Email", () => {
    expect(matchEmail(makeTextItem("  hello@resumebuilder.org  "))![0]).toBe(
      "hello@resumebuilder.org"
    );
  });
  ...
  it("Url", () => {
    expect(matchUrl(makeTextItem("  linkedin.com/in/resumebuilder  "))![0]).toBe(
      "linkedin.com/in/resumebuilder"
    );
  expect(matchUrl(makeTextItem("hello@resumebuilder.org"))).toBeFalsy();
  });
});
