...
export const resumeSlice = createSlice({
  ...
  reducers: {
    ...
    changeWorkExperiences: (
      ...
      // Ensure the work experience entry exists before assigning.
      while (draft.workExperiences.length <= idx) {
        const clone =
          typeof structuredClone === "function"
            ? structuredClone(initialWorkExperience)
            : JSON.parse(JSON.stringify(initialWorkExperience));
        draft.workExperiences.push(clone as any);
      }
    ...
    changeEducations: (
      ...
      const { idx, field, value } = action.payload;
      // Ensure the education entry exists before setting its field.
      // In some runtime flows (e.g. dynamic resumes or parsing), the
      // educations array may be shorter than the index being updated.
      // Safely expand the array with default entries so assignments
      // like `education.school = ...` won't throw.
      while (draft.educations.length <= idx) {
        // Use structuredClone (native) to copy the initialEducation template.
        // If structuredClone isn't available in the runtime, fallback to a
        // JSON clone to avoid mutating the template.
        const clone =
          typeof structuredClone === "function"
            ? structuredClone(initialEducation)
            : JSON.parse(JSON.stringify(initialEducation));
        draft.educations.push(clone as any);
      }
    ...
    changeProjects: (
      ...
      const { idx, field, value } = action.payload;
      // Ensure the project entry exists before assigning.
      while (draft.projects.length <= idx) {
        const clone =
          typeof structuredClone === "function"
            ? structuredClone(initialProject)
            : JSON.parse(JSON.stringify(initialProject));
        draft.projects.push(clone as any);
      }
